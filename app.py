...
def generate_receipt():
    ...
    total_funcionarios = len(admissionais) + len(demissionais) + len(
        periodicos)
    ...
    pdf.cell(200,
             10,
             txt=f"Empresa Cliente: {nome_empresa_cliente}",
             ln=True,
             align='C')
    ...
    pdf_output.seek(0)
    nome_pdf_baixado = f"{nome_empresa_cliente}-recibo.pdf"

    return send_file(pdf_output,
                     as_attachment=True,
                     download_name=nome_pdf_baixado,
